<template>
  <div class="flex flex-col gap-1" :class="[disabled && 'opacity-60']">
    <span v-if="label" :class="[hasError ? 'text-destructive dark:brightness-150' : 'text-foreground']">
      {{ label }}
    </span>
    <div
      ...
      :class="{ 'border-destructive bg-destructive/5 dark:bg-destructive/10': hasError, 'bg-muted/80 pointer-events-none': disabled }"
    >
      <slot />
    </div>
